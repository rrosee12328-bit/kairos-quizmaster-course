...
const Certificate = ({ userName, registrationNumber, courseCompletionDate, idType, lastSixDigits, exportMode, certificateId }: CertificateProps) => {
  ...
  return (
      ...
      {/* Date of Completion */}
      <div className="absolute top-[51.3%] left-[61.8%]">
        <p className={`${exportMode ? 'text-[20pt]' : 'text-[1.125rem]'} font-normal text-foreground leading-none`} style={{ letterSpacing: '0.02em', transform: 'translateY(-12px)', textShadow: exportMode ? '0 0 1px rgba(0,0,0,0.1)' : 'none' }}>{formatDate(courseCompletionDate)}</p>
      </div>
    </div>
  );
};
